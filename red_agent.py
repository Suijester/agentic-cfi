...
from tools import (
    ...
    write_file, dump_clang_ast, write_log
)
from prompts import RED_PROMPT
...
RED_SCHEMAS = [TOOL_SCHEMAS[t] for t in [
    ...
    "find_pointer_assignments", "write_file", "write_log",
    "write_policy", "compile_llvm_pass", "policy_to_llvm_pass"
]]

def run_red_agent(target_dir: str, max_steps: int = 20):
    ...
    messages = [
        {"role": "system", "content": RED_PROMPT},
        {"role": "user", "content": f"Analyze the C project in '{target_dir}' for forward-edge CFI."
        "Find all indirect call sites, write attack harnesses in attacks/ that bypass CFI checks, "
